...
class CNN_IMU_HR(nn.Module):
    ...
    def __init__(self, num_channels=3, kernel=(1,5), pool=(1,2), num_classes=12):
        ...
        self.layer1temp = nn.Sequential(
            nn.Conv2d(num_channels=1, C, kernel_size=kernel, stride=1, padding=(0,0)),
            # nn.BatchNorm2d(C),
            nn.ReLU(),
            nn.LocalResponseNorm(size=5, alpha=0.0001, beta=0.75),
...
#         self.list_IDs = list_IDs
  ...
  def __getitem__(self, index):
        ...
        plot = Image.open(self.path + ID + '.jpg')
...
dataset_train = pd.read_csv('/data/mark/NetworkDatasets/rgb_plots/Train/figure_labels.csv', ',', header=0)
dataset_pickle = '/data/mark/NetworkDatasets/baseline/pamap2.data'

train_path = '/data/mark/NetworkDatasets/rgb_plots/Train/'
...
dataset_validation = pd.read_csv('/data/mark/NetworkDatasets/rgb_plots/Validation/figure_labels.csv', ',', header=0)

Validation_path = '/data/mark/NetworkDatasets/rgb_plots/Validation/'
...
def train(optimizer, model, num_epochs, first_epoch=1):
    ...
    for epoch in range(first_epoch, first_epoch + num_epochs):
        ...
        # Save a checkpoint
        checkpoint_filename = '/home/mark/checkpoints/multi_channel_img_Dataset-{:03d}.pkl'.format(epoch)
